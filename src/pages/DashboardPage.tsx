...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const handleTradesUpdate = (e: CustomEvent) => {
      console.log('Trades update event detected, reloading trades...', e.detail);
      loadTrades();
    };
  ...
  const loadTrades = () => {
    try {
      ...
      if (csvTrades.length < 25) {
        console.log('Refreshing dashboard with new sample data...');
  ...
  // Simple metrics calculation for CSV data
  const calculateCSVMetrics = (trades: CSVTradeData[]) => {
    if (trades.length === 0) {
      return {
        totalTrades: 0,
        winRate: 0,
    ...
    const totalProfitLoss = trades.reduce((sum, t) => sum + t.result, 0);
  ...
  const metrics = calculateCSVMetrics(currentTrades);
