...
import { Eye, EyeOff, TrendingUp, ArrowRight, Shield, Zap, CheckCircle, Sparkles, Lock, User } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    signup(formData.email || 'user@tradient.com', formData.password || 'password', formData.name || 'User');
    navigate('/dashboard');
    setIsLoading(false);
  };
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <Card className="w-full bg-card/60 backdrop-blur-2xl border-border/30 shadow-[0_0_0_1px_rgba(255,255,255,0.1),0_20px_40px_-10px_rgba(0,0,0,0.4)] relative overflow-hidden">
          ...
          <CardHeader className="text-center pb-8 relative z-10">
            ...
            <p className="text-muted-foreground text-base mb-6">Start your trading journey today</p>
          ...
          <CardContent className="pt-0 relative z-10">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-3">
                <Label htmlFor="name" className="text-foreground text-sm font-semibold tracking-wide flex items-center gap-2">
                ...
                <div className="relative group">
                  <Input
                    ...
                    onBlur={() => setIsFocused('')}
                    className={`bg-background/40 border-border/40 text-foreground placeholder-muted-foreground/60 focus:border-primary/60 focus:ring-2 focus:ring-primary/20 transition-all duration-300 pr-4 h-12 rounded-lg backdrop-blur-sm ${isFocused === 'name' ? 'shadow-[0_0_20px_rgba(99,102,241,0.2)]' : ''}`}
                    required
                  />
                  <div className={`absolute inset-0 rounded-lg bg-gradient-to-r from-primary/10 to-transparent opacity-0 transition-opacity duration-300 ${isFocused === 'name' ? 'opacity-100' : ''} pointer-events-none`}></div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                {formData.password && (
                  <div className="mt-3 p-3 bg-muted/20 rounded-lg border border-border/20 backdrop-blur-sm">
                    <p className="text-xs font-medium text-foreground mb-2">Password requirements:</p>
                    <div className="space-y-1.5">
                      {passwordRequirements.map((req, index) => (
                        <div key={index} className="flex items-center gap-2 text-xs">
                          <div className={`w-4 h-4 rounded-full flex items-center justify-center ${req.met ? 'bg-primary/20' : 'bg-muted/40'}`}>
                            <CheckCircle className={`w-2.5 h-2.5 ${req.met ? 'text-primary' : 'text-muted-foreground'}`} />
                          </div>
                          <span className={req.met ? 'text-primary font-medium' : 'text-muted-foreground'}>
                            {req.text}
                          </span>
                        </div>
                      ))}
              ...
              <div className="space-y-3">
                <Label htmlFor="confirmPassword" className="text-foreground text-sm font-semibold tracking-wide">Confirm Password</Label>
                <div className="relative group">
                  <Input
                    id="confirmPassword"
                    ...
                    onBlur={() => setIsFocused('')}
                    className={`bg-background/40 border-border/40 text-foreground placeholder-muted-foreground/60 focus:border-primary/60 focus:ring-2 focus:ring-primary/20 pr-12 h-12 rounded-lg backdrop-blur-sm transition-all duration-300 ${isFocused === 'confirmPassword' ? 'shadow-[0_0_20px_rgba(99,102,241,0.2)]' : ''}`}
                    required
                  />
                  <div className={`absolute inset-0 rounded-lg bg-gradient-to-r from-primary/10 to-transparent opacity-0 transition-opacity duration-300 ${isFocused === 'confirmPassword' ? 'opacity-100' : ''} pointer-events-none`}></div>
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
                ...
                {formData.confirmPassword && formData.password !== formData.confirmPassword && (
                  <div className="flex items-center gap-2 text-destructive text-xs mt-2 p-2 bg-destructive/10 rounded-lg border border-destructive/20">
